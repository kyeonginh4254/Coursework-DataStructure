...
RBNode<T>* RBNode<T>::insert(std::unique_ptr<RBNode<T>>& n, const T& t) {
    ...
    if (t < n->key) insert(n->left, t);
    if (t > n->key) insert(n->right, t);
...
RBNode<T>* RBNode<T>::move_red_right(std::unique_ptr<RBNode<T>>& n) {
    n->flip_color();
    if (n->left && n->left->left && is_red(n->left->left)) {
        rotate_right(n);
        n->flip_color();
    }
    return n.get();
...
RBNode<T>* RBNode<T>::move_red_left(std::unique_ptr<RBNode<T>>& n) {
    std::cout << "Entering move_red_left with key: " << n->key << std::endl;
    n->flip_color();
    if (n->right && n->right->left && is_red(n->right->left)) {
        std::cout << "Performing rotate_right on right child." << std::endl;
        rotate_right(n->right);
        rotate_left(n);
        n->flip_color();
    }
    std::cout << "Exiting move_red_left with key: " << n->key << std::endl;
    return n.get();
}
